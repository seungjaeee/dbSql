...
--           (ex : emp테이블의 deptno(부서번호)
--                 dept테이블의 dept(부서번호))
--                 해당컬럼에 null 값은 들어 갈 수 없다

--NOT NULL : 데이터 입력시 해당 컬럼에 값이 반드시 들어와야 한다
--오라클 제약조건 이름을 임의로 생성(SYS-C000701 등)
...
--APPLICATION 레벨에서 체크 하도록 설계하는 경향이있다
...
INSERT INTO emp_test VALUES( 9998,'sally', 99); -- 이건가능
...
WHERE deptno = 99;
--부서정보를 지우려면
--지우려고하는 부서번호를 참조하는 직원정보를 삭제 또는 deptno 컬럼을 NULL 처리
--EMP테이블을 수정 후  -> DEPT
DROP TABLE emp_test;
...
--해당 데이터를 참조 하고 있는 EMP테이블의 dpetno 컬럼을 NULL로 변경
DELETE dept_test
WHERE deptno = 99;
rollback;
...
CREATE TABLE emp_test(
    ...
    DROP TABLE emp_test;
--table level로 CHECK 제약조건 제약조건 이름 생성
CREATE TABLE emp_test(
    empno NUMBER(4) PRIMARY KEY,   
    ename VARCHAR2(10),        
    sal NUMBER(7,2) ,       
...
--hp 컬럼의 타입을 VARCHAR2(20) -> VARCHAR2(30) 변경
--현재 emp_test 테이블에 ★데이터가 없기 때문에★ 컬럼 타입을 변경하는 것이 가능하다
ALTER TABLE emp_test MODIFY (HP VARCHAR(30));

--hp 컬럼의 타입을 VARCHAR2(20) -> NUMBER 변경
ALTER TABLE emp_test MODIFY (HP NUMBER);

DESC emp_test;

--컬럼명 변경
--해당 컬럼이 PK, UNIQUE, NOT NULL, CHECK제약조건시 기술한 컬럼명에 대해서도
-- 자동적으로 변경이 된다
--hp 컬럼 hp_n
...
--emp_test 테이블의 PRIMARY KEY 제약조건은 pk_emp_test 제약삭제
ALTER TABLE emp_test DROP CONSTRAINT pk_emp_test;

--테이블 컬럼이나 타입 변경은 제한적으로나마 가능
